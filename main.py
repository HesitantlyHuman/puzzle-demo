    ...
    match event.keycode:
        case 111:
            ...
            if len(steps) > 0:
                ...
                if has_won(blocks):
                    puzzles[current_puzzle_index] = (
                        puzzles[current_puzzle_index][0],
                        state.T,
                    )
                    puzzle_is_solved[current_puzzle_index] = True
                    next_puzzle_available = current_puzzle_index < len(puzzles) - 1
                    prev_puzzle_available = current_puzzle_index > 0
                    prev_level_button["state"] = (
                        "normal" if prev_puzzle_available else "disabled"
                    )
                    next_level_button["state"] = (
                        "normal" if next_puzzle_available else "disabled"
                    )
